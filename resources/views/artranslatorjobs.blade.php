...
<head>
    ...
    <meta name="description" content="we provide online translator jobs , we need translator spanish to english jobs and other languages , go down to see translator job description">
    <meta name="keywords" content="Financial Website Template, Bootstrap Template, Loan Product, Personal Loan">
    <title>online translator jobs|translating jobs|online translation jobs|translator jobs</title>
    <!-- Bootstrap -->
            ...
            <div class="row">
                <div class="col-md-6 col-sm-12 col-xs-12">
                    <div class="hero-captions">
                        <h2 class="hero-title " style="text-align: center;"> أملاء الأستمارة و سوف يتم التواصل معك لتصبح مترجمنا </h2>
                        <p class="hero-text c" style="text-align: center;">اذا كنت تبحث عن وظائف للترجمة لدوام كامل , العمل من البيت أو العمل لوقت جزئي أملاء الأستمارة   </p>
                ...
                <div class="col-md-5 col-md-offset-1 col-xs-12">
                    <div class="request-form">
                        <h2 class="al">يمكنك أن تنضم لينا</h2>
                        <p class="al c">أملاء الأستمارة و أنضم الي فريق المترجمين   </p>
                        <form method="post" action="{{url('online-job')}}">
                        {{ csrf_field() }}
                            <!-- Text input-->
                            <div class="form-group">
                                <label class="control-label sr-only" for="name">الأسم</label>
                                <input id="name" name="name" type="text" placeholder="الأسم" class="form-control input-md" value="{{old('name')}}" >

                                <p style="color:#f51f8a">{{$errors->first('name')}}</p>
                            </div>

                             <div class="form-group">
                                <label class="control-label sr-only" for="city">النوع</label>
                            ...
                            <!-- Text input-->
                            <div class="form-group">
                                <label class="control-label sr-only" for="email">البريد الألكتروني</label>
                                <input id="email" name="email" type="text" placeholder="البريد الألكتروني" class="form-control input-md" value="{{old('email')}}" >
                                <p style="color:#f51f8a">{{$errors->first('email')}}</p>
                            </div>
                            <!-- Text input-->
                            <div class="form-group">
                                <label class="control-label sr-only" for="phone">رقم الهاتف</label>
                                <input id="phone" name="phone" type="text" placeholder="رقم الهاتف" class="form-control input-md" value="{{old('phone')}}" >
                            ...
                            <!-- Text input-->
                            <div class="form-group">
                                <label class="control-label sr-only" for="phone">الغات من/الى</label>
                                <input id="phone" name="language" type="text" placeholder="الغات من/الى" class="form-control input-md" value="{{old('language')}}" >
                            ...
                            <!-- Text input-->
                            <div class="form-group">
                                <label class="control-label sr-only" for="email">عدد الكلمات اليومية</label>
                                <input id="email" name="capacity" type="text" placeholder="عدد الكلمات اليومية" class="form-control input-md" value="{{old('capacity')}}" >
                                <p style="color:#f51f8a">{{$errors->first('capacity')}}</p>
                            </div>


                             <div class="form-group">
                                <label class="control-label sr-only" for="city">نوع العمل</label>
                                <select id="city" name="jobtime" class="form-control">
                                    <option value="Full Time">دوام كامل</option>
                                    <option value="Freelance">عمل حر</option>
                                    <option value="Permanent">عمل مؤقت</option>
                                    <option value="Part Time">عمل بوقت جزئي</option>
                                    <option value="From Home">من البيت</option>
                                </select>
                            </div>


                            <div class="form-group">
                                <label class="control-label sr-only" for="name">التخصص</label>
                                <input id="name" name="speciality" type="text" placeholder="التخصص" class="form-control input-md"  value="{{old('speciality')}}">
                                <p style="color:#f51f8a">{{$errors->first('speciality')}}</p>
                            </div>
                            <!-- Text input-->
                            <div class="form-group">
                                <label class="control-label sr-only" for="email">عدد سنوات الخبرة</label>
                                <input id="email" name="experience" type="text" placeholder="عدد سنوات الخبرة" class="form-control input-md" value="{{old('experience')}}" >
                                <p style="color:#f51f8a">{{$errors->first('experience')}}</p>
                            </div>
                            <!-- Text input-->
                            <div class="form-group">
                                <label class="control-label sr-only" for="phone">العنوان </label>
                                <input id="phone" name="address" type="text" placeholder="العنوان " class="form-control input-md" value="{{old('address')}}">
                                <p style="color:#f51f8a">{{$errors->first('address')}}</p>
                            </div>
                            <!-- Select Basic -->

                          <div class="form-group">
                                <label >حمل السيرة الذاتية</label>
                            ...
                            <!-- Button -->
                            <div class="form-group">
                                <button type="submit" class="btn btn-default btn-block c">أرسال</button>
                            </div>
                        </form>
                    </div>
                </div>
